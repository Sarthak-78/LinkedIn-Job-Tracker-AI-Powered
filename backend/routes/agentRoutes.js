...
router.post('/search', async (req, res) => {
    try {
        ...
        taskDescription += `5. MATCHING PRIORITY: Check job TITLE first (40-50% match acceptable), then SKILLSET (90%+ required). If criteria met, accept immediately without checking other conditions.\n\n`;
        ...
        taskDescription += `   a) First, check if the job TITLE matches at least 40-50% of the target role: "${preferences.role}" (be flexible with variations)\n`;
        ...
        taskDescription += `   e) If title matches 40-50% AND skills match 90%+ in either initial or expanded view, immediately accept this job and stop searching.\n`;
